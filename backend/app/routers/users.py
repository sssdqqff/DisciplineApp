...
from backend.app.dependencies.auth import get_current_user  # dependency для текущего пользователя
...
def get_user_by_nickname(nickname: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    user_service = UserService(db)
...
def update_user(user_id: int, user: UserCreate, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
